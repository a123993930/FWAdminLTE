...
import com.guy.common.persistence.PropertyFilter.MatchType;
import com.guy.common.utils.Reflections;
...
public class HibernateDao<T, PK extends Serializable> extends
	 ...
	 */
	public List<T> findBy(final String propertyName, final Object[]  values,
			final MatchType matchType) {
		Criterion criterion = buildCriterion(propertyName, values, matchType);
		return find(criterion);
	}

	/**
	 * 关联查询 按属性过滤条件列表分页查找对象.
	 */
	public Page<T> findPageByDeta(final Page<T> page,
			final List<PropertyFilter> filters) {
		DetachedCriteria detachedCriteria = buildDetachedCriteriaByPropertyFilter(filters);
		return findPage(page, detachedCriteria);
	}
	...
	private Page<T> findPage(Page<T> page, DetachedCriteria detachedCriteria) {
		
		Criteria c = null;
		try {
			c = detachedCriteria.getExecutableCriteria(getSession());
		} catch (Exception e) {
	...
	protected Criterion buildCriterion(final String propertyName,
		...
		case EQ:
			...
			break;
		case LIKE:
			criterion = Restrictions.like(propertyName, (String) fieldValues[0],
					MatchMode.ANYWHERE);
			break;
		case LE:
...
//	}
	...
	/**
	 * 按属性条件列表创建DetachedCriteria,辅助函数.
	 */
	protected DetachedCriteria buildDetachedCriteriaByPropertyFilter(
			final List<PropertyFilter> filters) {
		...
		try {
			if (filters != null) {
                if (filters != null || filters.size() > 0) {
                    ResolveAlias ra = new ResolveAlias(detachedCriteria);
                    for (PropertyFilter propertyFilter : filters) {
                        if(!propertyFilter.hasMultiProperties()){//只有一个属性需要比较的情况.
                            String propertyName = propertyFilter.getPropertyName();
                            String realPropertyName = getRealPropertyName(propertyName,
                                    ra);
                            detachedCriteria.add(buildCriterion(realPropertyName,
                                    propertyFilter.getMatchValues(),
                                    propertyFilter.getMatchType()));
                        }else{//包含多个属性需要比较的情况,进行or处理
                            Disjunction disjunction = Restrictions.disjunction();  
                            for (String param : propertyFilter.getPropertyNames()) {
                                disjunction.add(Restrictions.or(buildCriterion(param,
                                        propertyFilter.getMatchValues(),
                                        propertyFilter.getMatchType())));
                            }
